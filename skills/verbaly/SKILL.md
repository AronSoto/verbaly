...
- **ICU syntax works but is not free, and the compiler handles that for you**: it ships the ICU parser only when a catalog actually uses it. Never hand-wire it. If messages reach the app after the build (a CMS, a fetched catalog) and may contain ICU, set `icu: true` in the config; otherwise such a message renders its own source and warns.
